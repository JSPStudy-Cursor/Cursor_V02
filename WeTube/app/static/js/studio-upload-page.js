...
(function () {
  ...
  const thumbnailPlaceholder = document.getElementById('upload-thumbnail-placeholder');
  ...
  if (form) {
    form.addEventListener('submit', function (e) {
      ...
      setTimeout(function () {
        const thumbnail = thumbnailInput && thumbnailInput.files[0];
        let msg = '프론트엔드 전용입니다. 실제 업로드 기능은 추후 구현됩니다.\n';
        msg += `제목: ${title}\n`;
        msg += `동영상: ${video.name} (${formatFileSize(video.size)})\n`;
        if (thumbnail) {
          msg += `썸네일: ${thumbnail.name}\n`;
        }
        msg += `공개 설정: ${document.getElementById('upload-visibility').value}`;
